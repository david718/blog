...
즉 state관련된 로직을 각 component 내부에서 정의해야 한다.  

React는 component 계층구조 중 상위에서 하위로만 data가 전달되므로  
어떤 component가 state를 소유해야할지 명확하지 않을 수 있다.  
아래 단계는 component와 state의 관계를 규정하는데 도움이 된다.  
  
모든 각 state 에게 아래 단계를 적용해보자

1. 해당 state에 기반한 data를 보여주는 모든 component를 찾자
2. 이 모든 component를 소유한 공동 component(계층구조의 상위component)를 찾자
3. 이러한 공동 component가 state를 소유해야 한다

> state를 소유할 적합한 component를 찾지 못했다면  
state를 소유하는 새로운 component를 만들고  
공동 component의 상위에 위치시키자

###제품 검색 App에서 위 단계를 적용하여 구현하기
구체적인 예시 제품 검색 App에 위 단계를 적용해보자
...
위 단계를 모두 완료하면 제품 검색 App에서 검색이 가능해졌음을 확인할 수 있다.  

##STEP5. 하위component에서 상위component의 state 수정이 가능하게 만들기
지금까지 props와 state를 부모에서 자식으로(Top-down) 전달하여  
data를 보여주는 App을 구현했다.  
이제 자식이 부모의 data(state)수정이 가능하도록 만들 차례다.  
Form component가 `FilterableProductTable`의 state를  
수정할 수 있도록 만들어야 한다.  
  
React는 부모에서 자식으로(한방향으로)만 data가 전달된다.  
여기에 몇줄의 코드를 추가하면  
...
이렇게 props로 setState를 전달하면 data 흐름을 파악하기 쉽다.  

##마무리
이 글을 통해서 어떻게 React로 component를 만들고 App을 구성할지  
아이디어를 얻었길 바란다.  
이전에 App을 구성했던 방식과는 조금 다를지 모르지만  
이러한 App 구성 방식은 훨씬 읽기 쉽고 모듈화된 코드가 된다.  
재사용성이 높기 때문에 코드가 간결해지는 것은 덤이다.  
