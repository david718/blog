...
###자바스크립트에서의 메모리 할당
변수 할당 시점에 메모리 필요량을 정해주지 않아도 알아서 할당한다
...
##메모리 누수
프로그램에서 사용했다가 더이상 필요하지 않게 된 메모리 중  
아직 회수되지 않은 메모리  
  
오직 개발자만이 어떠한 메모리를 회수해야 하는지 알 수 있음  
더이상 필요하지 않게 된 즉 참조하지 않는 경우를 체크해야 함!

##자바스크립트에서 메모리 누수 예시
자바스크립트에서는 `의도하지 않은 참조`때문에 메모리 누수가 발생한다
참조가 있으면 가비지 컬렉터가 변수에 할당된 메모리를 회수하지 않기때문  
  
이렇게 `의도하지 않은 참조`가 발생하는 경우는 아래 4가지가 있다
...
##결론
위 4가지 경우 자바스크립트에서 메모리 누수가 발생하기 쉬우므로  
의도하지 않은 참조를 주의하여 코딩한다면  
메모리 누수를 방지할 수 있을 것이다  